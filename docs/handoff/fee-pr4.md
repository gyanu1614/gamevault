...
| **promo 0** [start, start + 6 mo), R6 Credits / FC Points | **ea-sports-fc-26 / top_up** (FC Points are a bought-with-money currency; this catalogue files them under the `top-up` pair — the game has no `currency` pair) (1) | **r6-siege** — has ONLY an `account` pair, nothing holds R6 Credits; **fc-25**, **fc-26** — no such games. Matched by (slug, type ∈ top_up\|currency); never an account pair. |
...
Queried before deciding the promo pair (`game_categories` for r6-siege / ea-sports-fc-26 / fc-26 / fc-25):

| game_slug | category slug | type |
|---|---|---|
| ea-sports-fc-26 | buy-accounts | account |
| ea-sports-fc-26 | top-up | top_up |
| r6-siege | buy-accounts | account |
| fc-26, fc-25 | — | (no such game) |
...
| Check | Result |
|---|---|
| `fee-migration-parity.guard.integration` (rewritten) | **14/14** — one start date ≥ floor at 00:00 UTC; **405/405 pairs == `commissionPct()` at start − 1 s** and none to a PR 4 row; **405/405 pairs == the spec table at the start** (and at +24 h), all to PR 4 rows, none via fallback; six open-ended category rules only; PR 1 defaults closed exactly at the start; pair-rule set == spec ∩ catalogue with nothing unlisted; promo set == spec ∩ catalogue (= ea-sports-fc-26/top_up), never on an account pair, 0 at the start and the promo's last second, base (5) again the second it ends, today's rate before the start; ladder; settings; founding × 0.5 on both sides (7 → 10 base on an items pair); founding backfill intact |
| `fee-resolver.guard.integration` | **42/42** (was 31 + 1 new trigger case + 9 live-table cases + "15 days out" re-pointed) — items: legendary 7 before → 8.00 at the floor from the start (`floor_applied` false), gold 9, silver 9.5, diamond 8.5, bronze 10; unlisted currency 5 → 5, no step either side; listed currency (fisch) 5 → 10 headline, legendary 5 → 8; unlisted account 15 → 10 headline, legendary **13 already before the start** → 8; gta-v legendary 18 both sides, diamond 18.5; call-of-duty account 15 via its pair rule; listed top_up 5 → 10 / legendary 8, unlisted top_up 5 no step; founding beats rank at the start (5.00, rank_pts 0); FC Points promo: 5 before, 0 from the start for anonymous AND a legendary founding seller (founding on zero is a no-op), base 5 again after 6 months |
...
2. **Push on the day you want the clock to start** — the date is `push day + 15 days, 00:00 UTC` (floor 2026-10-06). Read the `NOTICE` lines in the push output: the start date, `closed 9 open-ended base rule(s)`, the matched/missing slugs per list, `… 6 category defaults, 25 pair rule(s), 1 promo(s)` (the promo NOTICE must show `matched {ea-sports-fc-26/top_up}`). If a list's "matched" set on prod differs from the table above, the prod catalogue differs from local — say so before announcing.
...
- **FC Points promo is on `ea-sports-fc-26` / `top_up`** (the spec's `fc-25`/`fc-26` do not exist; FC 26 has no `currency` pair — its bought-with-money points live on the `top-up` pair). 0% for six months from the start, then the top_up default 5. Never on the account pair (pinned).
...
- **R6 Credits promo: no row — nothing to put it on.** `r6-siege` has ONLY a `buy-accounts` (account) pair. If R6 Credits are to be sold, create the top_up (or currency) pair first via `ensureGameCategory`, then re-run the migration file by psql (idempotent, re-seed mode; promo rows are guarded by note) or insert the promo row by hand — promos need no notice.
