...
const CARDS: SellerCtaType[] = [
  ...
  },
  {
    ...
    // Number-free, per the fee-engine rule (no seller-fee figure in
    // marketing copy — fee-copy.guard.test.ts). The page passes main's
    // standard buyer/seller line in via `feeLine`; this is the fallback.
    sub: 'Every category priced separately.',
    tone: 'prototype',
...
export function SellerCta({ feeLine }: { feeLine?: string } = {}) {
  return (
      ...
      <ul className="mt-5 grid grid-cols-1 gap-3.5 lg:mt-6 lg:grid-cols-2 lg:gap-5 lg:pr-20">
        {CARDS.map((cta) => (
          <li key={cta.title} className={cta.layout === 'text-left' ? 'lg:col-span-2' : undefined}>
            {/* Card 2 (fees) carries the site's standard fee line when given. */}
            <SellerCtaCard cta={feeLine && cta.step === '02' ? { ...cta, sub: feeLine } : cta} />
          </li>
        ))}
      </ul>
    </section>
