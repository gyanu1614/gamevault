          ...
          <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="min-h-screen flex items-center justify-center p-4">
              <motion.div
                ...
                className="relative w-full max-w-2xl max-h-[85vh] overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0d0d14] shadow-2xl my-8"
              >
                {/* Header */}
                <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[0.06] bg-[#0d0d14]/95 backdrop-blur-xl px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Order #{order.order_number || order.id.slice(0, 8).toUpperCase()}</p>
                    </div>
                  </div>
                ...
                <div className="overflow-y-auto p-6" style={{ maxHeight: 'calc(85vh - 80px)' }}>
                  <div className="space-y-6">
                    {/* Item Image */}
                    {listing?.image_url ? (
                      <div className="relative w-full h-64 overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.02]">
                        <Image
                      ...
                      <div className="w-full h-64 rounded-xl border border-white/[0.08] bg-white/[0.02] flex items-center justify-center">
                    ...
                    <div className="space-y-6">
                      ...
                      {order.vaultshield_tier && (
                        <div className="rounded-lg border border-violet-500/20 bg-gradient-to-br from-violet-500/10 to-transparent p-4">
                      ...
                      {attrRows.length > 0 && (
                        <div>
                          <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Attributes</h4>
                      ...
                      {(listing?.platform || listing?.region) && (
                        <div>
                          <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Platform Details</h4>
                          <div className="space-y-2">
                            {listing.platform && (
                      ...
                      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Sold By</h4>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">@{seller?.username || 'unknown'}</p>
                          </div>
                          <Link
                            href={`/shop/${seller?.shop_slug || seller?.username}`}
                            target="_blank"
                            className="flex items-center gap-1.5 rounded-lg border border-violet-500/30 bg-violet-500/10 px-3 py-2 text-xs font-medium text-violet-400 transition-colors hover:border-violet-500/50 hover:bg-violet-500/20"
                          >
