...
/**
 * PERF-005 — the auth modal body, split out of AuthDialog.tsx and loaded with
 * next/dynamic from the provider.
 *
 * It pulls framer-motion, react-hook-form, zod, sonner, the lucide icon set and
 * AvatarUpload (@dicebear, a 131 kB chunk), none of which is needed until a
 * visitor actually opens the login modal — yet the provider lives in
 * providers.tsx, i.e. the ROOT LAYOUT, so all of it used to ship on first paint
 * for every route in the app.
 *
 * Full-screen Radix Dialog hosting a Login / Signup split-screen layout in the
 * LIGHT world used by the seller application
 * (src/app/account/become-seller/_redesign): ivory canvas, paper inputs,
 * forest-green primary, lime reserved for tiny accents only.
 *
 * Left column: light form (Login or Sign up, CSS-staggered reveal).
 * Right column (desktop only): hero photo under a forest scrim with a white
 *   tagline and a SafeDrop glass chip.
 *
 * On success the modal closes and `router.refresh()` runs so server components
 * re-render with the new auth state. No window.location reload.
 */
...
export default function AuthDialog({ open, onOpenChange, mode, onModeChange, redirectRef }: AuthDialogProps) {
