...
export function DisputeFilters() {
  ...
  const statusOptions = [
    ...
    { value: 'awaiting_seller_response', label: 'Awaiting Seller', color: 'bg-purple-500/20 text-purple-400 border-purple-500/30' },
    { value: 'awaiting_buyer_response', label: 'Awaiting Buyer', color: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30' },
  ]
  ...
  const priorityOptions = [
    ...
    { value: 'normal', label: 'Normal', color: 'bg-gray-500/20 text-gray-400 border-gray-500/30' },
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4">
      <div className="flex flex-col lg:flex-row gap-3">
        ...
        <form onSubmit={handleSearch} className="flex-1">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-white/[0.03] border border-white/[0.07] rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-violet-500/50 focus:border-violet-500/30 transition-all"
            />
          </div>
        </form>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-2 rounded-xl border border-white/[0.07] bg-white/[0.03] hover:bg-white/[0.06] text-sm text-gray-400 hover:text-white transition-colors flex items-center gap-2"
          >
            <Filter className="h-3.5 w-3.5" />
            Filters
            {activeFiltersCount > 0 && (
              <span className="px-1.5 py-0.5 text-[10px] font-bold bg-violet-500 text-white rounded-full min-w-[18px] text-center">
                {activeFiltersCount}
              </span>
            )}
          </button>
          ...
          {activeFiltersCount > 0 && (
            <button
              ...
              className="px-2.5 py-2 rounded-xl border border-white/[0.07] bg-white/[0.03] hover:bg-white/[0.06] text-gray-400 hover:text-white transition-colors"
            >
      ...
      {showFilters && (
        <div className="mt-3 pt-3 border-t border-white/[0.05] space-y-3">
          {/* Status Filters */}
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-gray-600 mb-2">Status</p>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive
                        ? option.color
                        : "bg-white/[0.03] text-gray-500 border-white/[0.07] hover:bg-white/[0.06] hover:text-gray-400"
                    )}
                  >
                    {option.label}
                  </button>
          ...
          {/* Priority Filters */}
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-gray-600 mb-2">Priority</p>
            <div className="flex flex-wrap gap-1.5">
