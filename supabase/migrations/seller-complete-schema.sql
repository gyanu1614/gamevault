-- =====================================================
-- GAMEVAULT SELLER DASHBOARD - COMPLETE SCHEMA
-- Date: January 24, 2026
-- Purpose: Create all tables for seller dashboard functionality
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.listings (
  ...
  -- Game & Category
  game text NOT NULL, -- valorant, league-of-legends, csgo, etc.
  category text NOT NULL, -- accounts, skins, boosting, coaching, in-game-items
  ...
  -- Delivery
  delivery_method text NOT NULL DEFAULT 'manual', -- instant, manual
  delivery_time text, -- "Instant", "1-2 hours", etc.
...
CREATE TABLE IF NOT EXISTS public.orders (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_number text UNIQUE NOT NULL, -- GV-123456
  ...
  -- Status
  status text NOT NULL DEFAULT 'pending', -- pending, processing, completed, disputed, cancelled, refunded

  -- Delivery
  delivery_details jsonb, -- { method: 'manual', notes: '...', credentials: '...' }
...
-- Set order number before insert if not provided
...
-- =====================================================
-- 4. SELLER STATS TABLE (for analytics)
-- =====================================================
CREATE TABLE IF NOT EXISTS public.seller_stats (
  ...
  -- Sales by Game (JSONB for flexibility)
  sales_by_game jsonb DEFAULT '{}', -- { "valorant": 5, "csgo": 3 }
...
-- =====================================================
-- 5. SELLER TIER PROGRESS TABLE
-- =====================================================
CREATE TABLE IF NOT EXISTS public.seller_tier_progress (
  seller_id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,

  -- Current Tier
  current_tier text NOT NULL DEFAULT 'bronze', -- bronze, silver, gold, platinum, diamond
...
CREATE TABLE IF NOT EXISTS public.seller_notifications (
  ...
  type text NOT NULL, -- new_order, new_review, message, payout, system
  title text NOT NULL,
  message text NOT NULL,

  -- Related Entity
  related_id uuid, -- order_id, review_id, etc.
  related_type text, -- order, review, message, etc.
...
CREATE TABLE IF NOT EXISTS public.seller_payouts (
  ...
  -- Payout Details
  amount decimal(10, 2) NOT NULL CHECK (amount > 0),
  method text NOT NULL DEFAULT 'paypal', -- paypal, stripe, bank_transfer
  destination text NOT NULL, -- email or account identifier
...
-- Function to calculate seller tier
